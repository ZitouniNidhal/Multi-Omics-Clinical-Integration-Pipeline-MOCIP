...
\title{A Scalable and FAIR-Compliant Multi-Omics Integration Pipeline with Automated FHIR R4 Clinical Semantic Mapping}
\titlerunning{FAIR-Compliant Multi-Omics Integration}

\author{Nidhal Zitouni \and Mohsen Maraoui}
\authorrunning{N. Zitouni and M. Maraoui}
\institute{Department of Computer Science, Faculty of Sciences of Monastir, Monastir, Tunisia \\
...
Looking ahead, the development of MOCIP will focus on enhancing its analytical depth and technical scalability. A primary objective is the integration of advanced Explainable AI (XAI) modules that can map high-dimensional latent factors back to specific biological pathways, providing clinicians with interpretable reasoning for every prediction. Furthermore, as genomic datasets expand toward pan-biobank scales, the framework will be migrated to a distributed computing architecture, such as Apache Spark, to maintain performance across hundreds of thousands of samples. We also intend to broaden the interoperability layer to support the HL7 FHIR R5 standard, ensuring that MOCIP remains at the cutting edge of clinical data exchange and precision oncology reporting.
\section{Conclusion}
